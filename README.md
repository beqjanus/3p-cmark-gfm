...
Push an intentional tag matching `cmark-gfm-*` (for example `cmark-gfm-master-499789b-1`) or use **Run workflow** for the release workflow. It builds all platforms, assembles releases through [`beqjanus/action-autobuild@v1.0.1`](https://github.com/beqjanus/action-autobuild/releases/tag/v1.0.1) and [`beqjanus/action-autobuild-release@v0.1.1`](https://github.com/beqjanus/action-autobuild-release/releases/tag/v0.1.1). It then publishes the GitHub release artifacts and copies the exact generated Autobuild `.zst`/`.tzst` archives to `fs_r2_deploy:buildsupport`. Configure the repository secret `RCLONE_CONFIG`; no credentials or rclone configuration are committed. Pull requests and ordinary branch pushes never reach the upload job.

The workflows use `actions/checkout@v7`, `actions/upload-artifact@v7`, and `TheMrMilchmann/setup-msvc-dev@v4`; each runs on Node 24. The pinned Autobuild action releases also use `actions/cache@v6`, `actions/upload-artifact@v7`, and Node 24.
